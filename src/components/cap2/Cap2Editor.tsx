...
export default function Cap2Editor({ projectId, projectName, prefill, warnings }: Cap2EditorProps) {
  ...
  function setIntroType(t: IntroType): void {
    setState((s) => ({ ...s, introType: t }));
  }
  ...
  async function handleGenerate(): Promise<void> {
    ...
    setErrorMsg(null);
    try {
      const response = await fetch(`/api/projects/${projectId}/cap2`, {
        method: "POST",
      ...
      <main className="flex flex-col gap-4">
        ...
        {activeSection?.isIntro ? (
          <IntroPanel
            ...
            onIntroTypeChange={setIntroType}
...
interface IntroPanelProps {
  ...
  onIntroTypeChange: (t: IntroType) => void;
  onUtmZoneChange: (z: UtmZone) => void;
}

function IntroPanel({ state, onIntroFieldChange, onIntroTypeChange, onUtmZoneChange }: IntroPanelProps) {
  const fields = state.introType === "MDIA" ? INTRO_FIELDS_MDIA : INTRO_FIELDS_DIA;
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-lg border border-stone-200 bg-white p-4">
        ...
        <div className="flex gap-2">
          {(["DIA", "MDIA"] as const).map((t) => (
            <button
              key={t}
              onClick={() => onIntroTypeChange(t)}
            ...
            >
              <span className="block text-base">{t}</span>
              <span className="block text-xs font-normal opacity-75">
                {t === "DIA" ? "Declaración de Impacto Ambiental" : "Modificación de la DIA"}
              </span>
            </button>
          ))}
        </div>
